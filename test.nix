...
let inherit (import ./.) unchecked checked self-checked-checked;
    tests = adt-lib:
      ...
      in { bad-unary = match (test-type.unary 1.1) { unary = x: x; multiary = { foo }: foo; };
           extra-multiary = test-type.multiary { foo = 1; bar = true; };
           missing-multiary = test-type.multiary {};
           bad-multiary = match (test-type.multiary { foo = 1.1; }) { unary = x: x; multiary = { foo }: foo; };
           bad-ctor-name = (make-type "test.bad-ctor-name" { _val = null; })._val;
           bad-arity = (make-type "test.bad-arity" { foo = 1; }).foo;
           bad-arg-name = (make-type "test.bad-arg-name" { foo = { _val = int; }; }).foo;
           bad-arg-type = (make-type "test.bard-arg-type" { foo = { bar = 1; }; }).foo;
